...
public class UniController : MonoBehaviour, ITaskContoller
{
    ...
    void Update(){
        ...
        if(Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift))
        {
            ...
            if ((inputFields[0].GetComponent<Text>().text.ToLower().Equals($"{gameManager.PlayerName}@lmu.de") || inputFields[0].GetComponent<Text>().text.ToLower().Equals($"{gameManager.PlayerName}@campus.lmu.de") || inputFields[0].GetComponent<Text>().text.ToLower().Equals($"{gameManager.PlayerName}@med.lmu.de")) && (inputFields[1].transform.parent.gameObject.GetComponent<InputField>().text.ToLower().Replace(" ", "").Equals(gameManager.NewPassword) || inputFields[1].transform.parent.gameObject.GetComponent<InputField>().text.Equals(gameManager.backupPassword)))
            {
                gameManager.Logger.LogData(this, LogType.Task, "Correct email and password given" );
                Debug.Log("Email and password given");
                loginDone = true;
                inputFieldCounter = 2;
                ...
                if (Regex.IsMatch(inputFields[2].GetComponent<Text>().text, @"^\d+$"))
                {
                    if(ErrorMessageVisible)
                {
                    ...
                    gameManager.Logger.LogData(this, LogType.Task, "Security Question submitted successfully" );
                    SecurityQuestionDone = true;
                    submitButton.onClick.Invoke();         
                }else
                {
                    gameManager.Logger.LogData(this, LogType.Task, "Matriculation Number has unallowed characters" );
                    ErrorMessage1.GetComponent <TextMeshProUGUI>().color = new Color(255f,255f,255f,1f);
                    ErrorMessageVisible = true;
                }
    ...
    IEnumerator EndTask()
    {
     ...
     public void SetNotificationVisible(bool isVisible){
        ...
        gameManager.Logger.LogData(this, LogType.Task, "TNotification Lock set" );
